/* ============================================
   LIFT — Fine sessione (2 step)
   1) mood/energia/note
   2) risultato (PR + volume + durata + nota personale)
   Nessun feedback AI: mood + energia + nota e via.
   ============================================ */
...
function _wireStep1() {
  ...
  document.getElementById("fin-save").onclick = _goSave;
}

/* ---------- STEP 2: spinner salvataggio ---------- */

function _finStep2Html() {
  return `
    <div class="fin-analyzing">
      <div class="fin-spin"></div>
      <div class="fin-analyzing-label">Salvataggio in corso…</div>
    </div>
  `;
}
...
function _renderStep3() {
  ...
  const note = (finState.notes || "").trim();
  const noteHtml = note
    ? `<div class="fin-note-recap">
        <div class="fin-note-label">La tua nota</div>
        <div class="fin-note-text">${escapeHtml(note).replace(/\n/g, "<br>")}</div>
       </div>`
    : "";
